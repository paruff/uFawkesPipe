...
def _language_image(language: str) -> str:
    if language not in _LANGUAGE_IMAGES:
        raise ContractError(f"unsupported app.language '{language}' — supported: {', '.join(sorted(_LANGUAGE_IMAGES))}")
...
def _language_command(stage: dict, language: str, stage_name: str) -> str:
    for entry in stage.get("commands", []):
        if entry.get("language") == language:
            return entry["cmd"]
    raise ContractError(f"stages.{stage_name}.commands has no entry for app.language '{language}'")
...
def _sast_step(contract: dict) -> dict:
    return {"name": "sast", "image": "sonarsource/sonar-scanner-cli:latest", "commands": ["sonar-scanner"]}


def _dependency_scan_step(contract: dict) -> dict:
    return {"name": "dependency-scan", "image": "aquasec/trivy:latest", "commands": ["trivy fs --exit-code 1 ."]}
...
def _build_step(contract: dict) -> dict:
    ...
    if builder == "cnb":
        cnb_builder = build_cfg.get("cnb", {}).get("builder", "paketobuildpacks/builder:base")
        ...
        raise ContractError(f"unsupported build.builder '{builder}' — supported: cnb, docker")
    return {"name": "build", "image": "docker:24-cli", "commands": [cmd]}


def _image_scan_step(contract: dict) -> dict:
    return {"name": "image-scan", "image": "aquasec/trivy:latest", "commands": ["trivy image --exit-code 1 $CI_REPO_NAME"]}


def _push_step(contract: dict) -> dict:
    return {"name": "push", "image": "docker:24-cli", "commands": ["docker push $CI_REPO_NAME"]}
...
def render(contract: dict) -> str:
    ...
    steps = [_STEP_BUILDERS[name](contract) for name in STAGE_ORDER if _stage_enabled(contract, name)]
    if not steps:
        raise ContractError("no stages are enabled in .fawkespipe.yml — nothing to build")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--check", action="store_true", help="Check for drift without writing --output")
